...
void processOptions(byte *buffer) {
  if (strcmp((char*)buffer,"-com")==0) outputMode=OUTPUT_COM;
  if (strcmp((char*)buffer,"-cmd")==0) outputMode=OUTPUT_CMD;
  }

byte *scanHex(byte *buffer,word *dest) {
  *dest=0;
...
void process_data_line(byte *buffer) {
  word addr;
  word wrd;
  buffer++;
...
void process_start_line(byte *buffer) {
  buffer++;
  buffer=scanHex(buffer,&startAddress);
  }

void doSymbol(byte *sym,word addr,byte valid) {
  int i;
  int symb;
  symb=-1;
  i=0;
  while (i<numSymbols) {
    if (strcmp(symbols[i],(char*)sym)==0) { symb=i; i=numSymbols; }
    i++;
    }
  if (symb>-1) {
    if (valid!=0) {
    ...
    strcpy(symbols[numSymbols],(char*)sym);
    symbola[numSymbols]=addr;
    symbolv[numSymbols++]=valid;
    }
  }

word getSymbolValue(byte *sym) {
  int i;
  int symb;
  symb=-1;
  i=0;
...
void readPass() {
  ...
  for (i=0;i<numObjects;i++) {
    strcpy((char*)buffer,objects[i]); strcat((char*)buffer,".prg");
    if ((source=fopen((char*)buffer,"r"))==NULL) {
      printf("Could not open source file: %s\n",buffer);
      exit(1);
      }
    while (fgets((char*)inputLine,1024,source)!=0) {
      while (inputLine[0]!='\0' && inputLine[strlen((char*)inputLine)-1]<32)
        inputLine[strlen((char*)inputLine)-1]='\0';
      if (inputLine[0]=='?') {
...
void resolvePass() {
  ...
  while (fgets((char*)inputLine,1024,source)!=0) {
    ...
    if (inputLine[0]=='?') {
      ...
      fgets((char*)inputLine,1024,source);
      while (inputLine[0]!='\0' && inputLine[strlen((char*)inputLine)-1]<32)
        inputLine[strlen((char*)inputLine)-1]='\0';
      if (inputLine[0]!=':') {
...
void binaryPass() {
  strcpy((char*)buffer,"rclink2.$$$");
  if ((source=fopen((char*)buffer,"r"))==NULL) {
    printf("Could not open source file: %s\n",buffer);
    exit(1);
    }
  strcpy((char*)buffer,objects[numObjects-1]);
  switch (outputMode) {
    case OUTPUT_COM:strcat((char*)buffer,".com"); break;
    case OUTPUT_CMD:strcat((char*)buffer,".cmd"); break;
    }
  if ((file=fopen((char*)buffer,"wb"))==NULL) {
    printf("Could not open output file: %s\n",buffer);
    exit(1);
    }
  while (fgets((char*)inputLine,1023,source)!=0) {
...
int main(int argc,char *argv[]) {
  ...
  numSymbols=0;
  for (i=1;i<argc;i++) {
    if (argv[i][0]=='-') processOptions((byte*)argv[i]);
